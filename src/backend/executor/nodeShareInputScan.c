...
char *joint_lock_file_name(ShareInput_Lk_Context *lk_ctxt, char *name)
{
	...
	{
		strncat(lock_file, lk_ctxt->lkname_ready, strlen(lk_ctxt->lkname_ready));
	}
	else
	{
		strncat(lock_file, lk_ctxt->lkname_done, strlen(lk_ctxt->lkname_done));
	}
	strncat(lock_file, name, strlen(name));
	return lock_file;	
}
